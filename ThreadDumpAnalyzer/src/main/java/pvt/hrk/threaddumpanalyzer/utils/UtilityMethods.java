...
public interface UtilityMethods {
	Predicate<String> isNotEmpty = line -> line != null
	...
	Function<LocalDateTime, java.sql.Timestamp> convertToSQLDateTime = localDateTime -> {
	...
	Function<Path, List<String>> readAllNonEmptyLines = p -> {
		try {
			List<String> contents = FileUtils.readLines(p.toFile(), "UTF-8").stream().filter(isNotEmpty)
